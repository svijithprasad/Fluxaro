...
const TicketForm = ({ getNewTicket, laneId, subaccountId }: Props) => {
  ...
  useEffect(() => {
    if (defaultData.ticket) {
      ...
      // FIX: Add this line to ensure the Assigned User is updated when editing!
      if (defaultData.ticket.assignedUserId) {
        setAssignedTo(defaultData.ticket.assignedUserId);
      } else if (defaultData.ticket.Assigned?.id) {
        setAssignedTo(defaultData.ticket.Assigned.id); // Fallback depending on your Prisma include
      } else {
  ...
  const onSubmit = async (values: z.infer<typeof TicketFormSchema>) => {
    ...
    if (!laneId) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Missing Lane ID. Cannot save ticket.",
      });
      console.error("SUBMIT FAILED: laneId is undefined");
      return;
    }
    try {
