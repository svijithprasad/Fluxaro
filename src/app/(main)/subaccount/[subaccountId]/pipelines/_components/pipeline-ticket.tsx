...
const PipelineTicket = ({
  ...
  const handleClickEdit = async () => {
    setOpen(
      <CustomModal title="Update Ticket Details" subheading="Modify ticket information">
        <TicketForm
          key={ticket.id}
          getNewTicket={editNewTicket}
          laneId={laneId} // <-- FIX: Use the new prop instead of ticket.laneId
      ...
      {(provided, snapshot) => (
        <div
          ...
          className={cn(
            ...
            snapshot.isDragging && "rotate-[3deg] scale-[1.05] z-50"
          )}
        >
          <AlertDialog>
            <DropdownMenu>
              <Card
                className={cn(
                  "my-2 dark:bg-slate-900 bg-white shadow-none transition-all cursor-grab active:cursor-grabbing",
                  snapshot.isDragging
